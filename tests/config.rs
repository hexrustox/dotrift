...
fn read_assembles_desired_deployment_from_config_and_data() {
    ...
    let mut deployment = config::read(&source, Some(target.clone()), env.env(), false)
...
fn effective_symlink_drops_mode_from_earlier_rule() {
    ...
    report::clear();
    let deployment = config::read(&source, Some(target.clone()), env.env(), false)
        .expect("cannot read configuration");
...
fn active_profile_without_definition_is_ignored() {
    ...
    let deployment = config::read(&source, Some(env.target_dir()), env.env(), false)
        .expect("cannot read configuration");
...
fn missing_template_variable_fails_before_parsing() {
    ...
    let error = config::read(&source, Some(env.target_dir()), env.env(), false)
...
fn read_fails_with_expected_error(setup: impl FnOnce(&TestEnv), expected: &[&str]) {
    ...
    let error = config::read(&env.source_dir(), Some(env.target_dir()), env.env(), false)
...
fn relative_target_directory_is_rejected() {
    ...
    let error = config::read(&source, None, env.env(), false)
...
#[test_case(
    |env| {
        fs::create_dir_all(env.source_dir().join("config/editor")).unwrap();
        fs::write(env.source_dir().join("config/editor/x.toml"), b"x").unwrap();
...
fn portal_mapping_yields_expected_entries(setup: impl FnOnce(&TestEnv) -> Vec<DeploymentEntry>) {
    ...
    let deployment = config::read(&env.source_dir(), Some(env.target_dir()), env.env(), false)
        .expect("cannot read configuration");
...
fn target_inside_source_reports_overlap() {
    ...
    let error = config::read(&source, Some(source.join("inside")), env.env(), false)
