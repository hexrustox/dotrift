...
use super::scenario::ApplyScenario;
...
pub const MAX_DIR_DEPTH: usize = 2;

pub fn name_strategy() -> impl Strategy<Value = OsString> {
...
pub fn file_count<T>(node: &Node<T>) -> usize {
    match node {
        Node::File(_) => 1,
...
pub fn count_files_on_disk(root: &Path) -> usize {
    let mut n = 0;
...
pub fn world_strategy() -> impl Strategy<Value = (Node<Vec<u8>>, Node<PathBuf>)> {
...
pub fn cleanup_world_strategy()
-> impl Strategy<Value = (Node<Vec<u8>>, Node<PathBuf>, Node<PathBuf>)> {
...
pub fn render_portals(target: &Node<PathBuf>) -> String {
...
pub fn materialize(source_dir: &Path, tree: &Node<Vec<u8>>) {
    write_node(source_dir, tree);
}

pub fn assert_symlink_tree(
    source: &Path,
    target: &Path,
    db: &StateDatabase,
    node: &Node<PathBuf>,
...
pub fn parse_actions(output: &str) -> Vec<(Action, PathBuf)> {
    output
        .lines()
        .map(|line| {
