...
use common::{
    Action, ApplyScenario, Prompt, assert_symlink_tree, cleanup_world_strategy,
    count_files_on_disk, dry_run_output, file_count, materialize, render_portals, world_strategy,
};
...
proptest! {
    ...
    #[test]
    fn apply_cleanup_prune_empties_target((source_tree, target_tree) in world_strategy()) {
        let scenario = ApplyScenario::new(|_, _| "");
        materialize(&scenario.source, &source_tree);
        scenario.write_config(&render_portals(&target_tree));
        scenario.run();
        ...
        prop_assert!(scenario.target.is_dir());
        prop_assert_eq!(db.managed_paths().unwrap().len(), 0);
    }

    #[test]
    fn apply_dry_run_matches_real_deploy((source_tree, target_tree) in world_strategy()) {
        let scenario = ApplyScenario::new(|_, _| "");
        materialize(&scenario.source, &source_tree);
        scenario.write_config(&render_portals(&target_tree));
        ...
        prop_assert_eq!(count_files_on_disk(&scenario.target), reported.len());
    }

    #[test]
    fn apply_dry_run_cleanup_matches_removals(
        (source_tree, target_tree, pruned_tree) in cleanup_world_strategy(),
    ) {
        let scenario = ApplyScenario::new(|_, _| "");
        materialize(&scenario.source, &source_tree);
