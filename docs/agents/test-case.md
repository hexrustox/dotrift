...
prefer the `; "label"` form. Multiword labels must be `snake_case` — the
label becomes the generated test fn identifier. Each attribute expands to
exactly one `#[test]`, so `cargo test` counts grow accordingly; pick labels
that survive a CI search.
