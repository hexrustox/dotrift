...
parsed as plain TOML (ADR-0003). No environment variables are injected
implicitly.
...
4. Equal timestamps are ordered lexicographically by profile name; the
   lexicographically later name wins the tie. dotrift's own activations are
   strictly ordered (see `spec/core.md § active_profiles Table`), so this
   tie-break guards state written outside dotrift.
