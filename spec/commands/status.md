...
1. Open the state database read-only (see `spec/core.md § State database`).
   A missing database is a valid empty state with no records.
...
4. Print one line per record, sorted by target path, comparing paths
   component-wise (see `spec/dotrift-toml.md § Path rules`).
...
- The target path is the recorded target path, displayed per the path display
  convention (`spec/commands/global.md § Output conventions`).
...
The column layout is identical with colors on and off: the verdict word is
padded before being colored. Lines are sorted component-wise by target path.
A database with no records prints nothing.
...
global source-directory requirement (see `spec/commands/global.md § Source
directory requirement`) does not apply.
