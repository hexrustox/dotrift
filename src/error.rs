...
macro_rules! read_file_err {
    ($result:expr, $path:expr) => {
        $result.wrap_err_with(|| format!("Failed to read file `{}`", $path.display()))
    };
}
...
macro_rules! read_link_err {
    ($result:expr, $path:expr) => {
        $result.wrap_err_with(|| format!("Failed to read symlink `{}`", $path.display()))
    };
}
...
macro_rules! read_dir_err {
    ($result:expr, $path:expr) => {
        $result.wrap_err_with(|| format!("Failed to read directory `{}`", $path.display()))
    };
}
...
macro_rules! create_dir_err {
    ($result:expr, $path:expr) => {
        $result.wrap_err_with(|| format!("Failed to create directory `{}`", $path.display()))
    };
}
...
macro_rules! remove_dir_err {
    ($result:expr, $path:expr) => {
        $result.wrap_err_with(|| format!("Failed to remove directory `{}`", $path.display()))
    };
}
...
macro_rules! parse_err {
    ($result:expr, $path:expr) => {
        $result.wrap_err_with(|| format!("Failed to parse file `{}`", $path.display()))
    };
}
...
macro_rules! glob_err {
    ($result:expr, $pattern:expr) => {
        $result.wrap_err_with(|| format!("Invalid glob pattern: `{}`", $pattern))
    };
}
